...
import 'chat_room_helpers.dart'; // ← 이거 추가
...
class _ChatRoomViewState extends State<_ChatRoomView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ctrl.onShowCalendarBlockedDialog = () => _showCancelBlockedByCalendarDialog(ctrl);
  ...
  Map<String, dynamic> _normalizeJobMap(Map<String, dynamic> m) {
    dynamic pick(List keys) {
      for (final k in keys) { if (m[k] != null) return m[k]; }
      return null;
    }
  ...
  void _openWorkerCalendar(ChatRoomController ctrl) {
    ...
    Navigator.pushNamed(context, '/worker-calendar', arguments: {
      'focusJobId': jobId,
      'focusTitle': jobTitle,
      'fromChatRoom': true,
    });
  ...
  void _goReview(ChatRoomController ctrl) {
    ...
    final jobTitle = (src['title'] ?? src['job_title'] ?? '').toString().trim();
    final companyName =
        (src['client_company_name'] ?? src['company'] ?? '기업').toString().trim();
                ...
                child: Column(
                  ...
                  children: [
                    const Icon(Icons.rate_review_rounded,
                        size: 48, color: Color(0xFF1675F4)),
  ...
  Widget build(BuildContext context) {
    ...
    if (ctrl.userType == 'client') {
      ...
      final wId = int.tryParse(
          widget.jobInfo['worker_id']?.toString() ?? '');
      if (wId != null) {
        onProfileTap = () =>
      ...
      final cId = int.tryParse(
          widget.jobInfo['client_id']?.toString() ?? '');
      if (cId != null) {
        onProfileTap = () =>
    ...
    final src = ctrl.jobSource;
    final jobTitle =
        (src['title'] ?? src['job_title'] ?? '').toString().trim();
              ...
              if (!ctrl.isLoadingJobInfo)
                ChatRoomJobPanel(
                  ...
                  onAddToCalendar: () async {
                    ctrl.workLoading = true;
                    ctrl.notifyListeners();
                    try {
                      final ok = await ctrl.confirmStartWork();
                      if (ok) ctrl.onShowSnackbar?.call('캘린더에 등록했어요.');
                    } finally {
                      ctrl.workLoading = false;
                      ctrl.notifyListeners();
                    }
                  },
                            ...
                            child: Row(
                              children: [
                                ...
                                IconButton(
                                  ...
                                  color: ctrl.inputEnabled
                                      ? const Color(0xFF4B5563)
                                      : const Color(0xFFD1D5DB),
                                  onPressed: ctrl.inputEnabled
                                      ? () => _pickAndSendImage(ctrl)
                                      : null,
                                ),
                                const SizedBox(width: 2),
                                GestureDetector(
                                  onTap: ctrl.inputEnabled
                                      ? () => _sendMessage(ctrl)
                                      : null,
                                  child: Container(
