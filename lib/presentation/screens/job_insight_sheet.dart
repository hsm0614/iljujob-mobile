...
class _JobInsightSheetState extends State<JobInsightSheet> {
  ...
  static const _border = Color(0xFFE5E8EB);
  static const _text   = Color(0xFF191F28);
  static const _label  = Color(0xFF8B95A1);
  ...
  Widget _buildContent(ScrollController ctrl) {
    ...
    return ListView(
      ...
      children: [
        ...
        // ── 수치 카드 3개 ──
        Row(children: [
          _statCard('조회수',  '$views회',     Icons.remove_red_eye_outlined, Colors.grey),
          const SizedBox(width: 10),
          _statCard('지원자',  '$applies명',   Icons.people_outline, _blue),
          const SizedBox(width: 10),
          _statCard('북마크',  '$bookmarks개', Icons.bookmark_outline, Colors.purple),
        ]),
        const SizedBox(height: 16),
          ...
          Container(
            ...
            child: Column(children: [
              Row(
                ...
                children: [
                  const Text('조회→지원 전환율',
                    style: TextStyle(fontSize: 13, color: _label)),
                  Text('${applyRate}%',
                    style: TextStyle(
                      fontSize: 15, fontWeight: FontWeight.w800,
                      color: applyRate >= d.benchApplyRate
                ...
                child: LinearProgressIndicator(
                  ...
                  valueColor: AlwaysStoppedAnimation(
                    applyRate >= d.benchApplyRate
                        ? Colors.green.shade400 : Colors.orange))),
              const SizedBox(height: 6),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('업종 평균 ${d.benchApplyRate}%',
                    style: const TextStyle(fontSize: 11, color: _label)),
                  Text(
                    d.rateVsBench == 'above' ? '평균보다 높아요 🔥'
                    : d.rateVsBench == 'below' ? '평균보다 낮아요'
                    : '평균 수준',
                    style: TextStyle(
                      fontSize: 11, fontWeight: FontWeight.w600,
                      color: d.rateVsBench == 'above'
                          ? Colors.green.shade600 : _label)),
                ]),
            ]),
          ),
          const SizedBox(height: 16),
        ...
        if (myHourly > 0 && avgHourly > 0) ...[
          ...
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 8),
              _compareRow('평균 시급', '${fmt.format(avgHourly)}원',
                payLevel == 'low'  ? Colors.red
                : payLevel == 'high' ? Colors.green
                : Colors.orange),
              const SizedBox(height: 10),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: _border)),
          child: Row(children: [
            Expanded(child: _bigNumber(
              '$applies명', '내 공고 지원자',
              applies >= avgApplicants ? Colors.green.shade600 : Colors.orange)),
            Container(width: 1, height: 50, color: _border),
            Expanded(child: _bigNumber(
              '$avgApplicants명', '업종 평균 지원자', Colors.grey)),
          ]),
        ),
        const SizedBox(height: 16),
        ...
        if (d.messages.isNotEmpty) ...[
          ...
          Container(
            ...
            child: Column(
              ...
              children: d.messages.map((m) => Padding(
  ...
  Widget _compareRow(String label, String value, Color? vc) =>
  ...
  Widget _payCompareBar(int myH, int avgH) {
    ...
    return Column(children: [
      ...
      const SizedBox(height: 6),
      Row(children: [
        _dot(_blue),   const SizedBox(width: 4),
  ...
  Widget _payLevelBadge(String level, int myH, int avgH, NumberFormat fmt) {
    ...
    switch (level) {
      case 'high':
        ...
        bg = Colors.green.withOpacity(0.1); fg = Colors.green.shade700; break;
      case 'low':
        text = '평균보다 ${fmt.format(diff)}원 낮아요 📢 시급 인상을 고려해보세요';
        bg = Colors.red.withOpacity(0.1);   fg = Colors.red.shade700;   break;
      default:
        text = '평균과 비슷한 시급이에요 👍';
        bg = Colors.orange.withOpacity(0.1); fg = Colors.orange.shade700;
    }
    return Container(
      width: double.infinity,
  ...
  Widget _bigNumber(String value, String label, Color color) =>
    Column(children: [
