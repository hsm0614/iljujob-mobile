...
class _AiJobWizardState extends State<AiJobWizard>
  ...
  Future<void> _nextQ() async {
    ...
    setState(() { _q++; _customHours = false; _customWage = false; });
    _fadeCtrl.forward();
  }

  Future<void> _prevQ() async {
    if (_q == 0) { widget.onSkip(); return; }
    await _fadeCtrl.reverse();
    setState(() => _q--);
    _fadeCtrl.forward();
  }
  ...
  Future<void> _onComplete() async {
    ...
    for (final c in _allCats) {
      if (c.sub.contains(_category)) { majorName = c.name; break; }
    }

    // 제목용 지역 (구/시 단위)
    final shortCity = _locationCity.isNotEmpty
        ? _locationCity
    ...
    setState(() { _isGenerating = true; _generateError = null; });
...
제목:''';
  ...
  Widget _buildQuestion() {
    switch (_q) {
      case 0: return _buildCategoryQ();
      case 1: return _buildLocationQ();
      case 2: return _buildDateQ();
      case 3: return _buildHoursQ();
      case 4: return _buildWageQ();
  ...
  Widget _buildCategoryQ() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GridView.count(
        ...
        crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: 1.05,
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text(cat.name,
                  ...
                  style: TextStyle(
                    ...
                    color: isSel ? Colors.white : isOpen ? _blue : _sub,
                    height: 1.3,
                  )),
            ...
            : Builder(builder: (_) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(color: _blue.withOpacity(0.12), width: 1),
                  ),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    Row(children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Icon(Icons.close_rounded, size: 16, color: _label)),
                    ]),
                    const SizedBox(height: 10),
                    Wrap(spacing: 7, runSpacing: 7,
                      children: cat.sub.map((s) {
                        final sel = _category == s;
                        return GestureDetector(
                          onTap: () {
                            setState(() { _category = s; _majorCat = ''; });
                            // 소분류 선택 즉시 다음으로
                            Future.delayed(const Duration(milliseconds: 200), _nextQ);
                          },
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(99),
                              border: Border.all(
                                color: sel ? _blue : const Color(0xFFDDE3EC),
                                width: sel ? 0 : 1,
                              ),
                              boxShadow: sel ? [] : [
        ...
        child: Container(
          ...
          child: Row(children: [
            Text('📍', style: TextStyle(
                fontSize: 22, color: _location != null ? _blue : _label)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                children: [
              Text('근무지 주소', style: TextStyle(
                  fontSize: 11, fontWeight: FontWeight.w600,
                  color: _location != null ? _blue : _label)),
              const SizedBox(height: 3),
              Text(_location ?? '주소를 검색해주세요', style: TextStyle(
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: sel ? _blue : _bg,
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: sel ? _blue : _border, width: sel ? 2 : 1),
              ),
              child: Column(children: [
                ...
                Text(fmt.format(d), style: TextStyle(
                    fontSize: 11, color: sel ? Colors.white70 : _label)),
              ]),
            ),
          );
        }).toList(),
        ...
        child: Container(
          ...
          child: Row(children: [
            Icon(Icons.calendar_today_rounded,
                size: 18, color: _startDate != null ? _blue : _label),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Wrap(spacing: 10, runSpacing: 10,
        children: presets.map((h) {
          ...
          return GestureDetector(
            onTap: () {
              setState(() { _workHours = h; _customHours = false; });
            ...
            child: AnimatedContainer(
              ...
              child: Column(children: [
                Text('$h시간', style: TextStyle(
                    fontSize: 18, fontWeight: FontWeight.w800,
                    color: sel ? Colors.white : _text)),
                const SizedBox(height: 2),
                Text('09:00 ~ ${9 + h}:00', style: TextStyle(
                    fontSize: 11, color: sel ? Colors.white70 : _label)),
              ]),
            ),
          );
      ...
      if (_customHours) ...[
        const SizedBox(height: 12),
        TextField(
          controller: _hoursCtrl, keyboardType: TextInputType.number,
          autofocus: true,
          decoration: InputDecoration(
            hintText: '예) 5', suffixText: '시간',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 10, runSpacing: 10,
        children: presets.map((p) {
          ...
          return GestureDetector(
            onTap: () {
              setState(() { _hourlyWage = p['wage'] as int; _customWage = false; });
              Future.delayed(const Duration(milliseconds: 180), _onComplete);
            },
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 180),
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: sel ? _blue : _border, width: sel ? 2 : 1),
                boxShadow: sel ? [BoxShadow(color: _blue.withOpacity(0.2),
                    blurRadius: 8, offset: const Offset(0, 4))] : [],
              ),
              child: Column(children: [
                Text('일급 ${fmt.format(p['pay'])}원',
                  style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800,
                      color: sel ? Colors.white : _text)),
                const SizedBox(height: 2),
                Text('시급 ${fmt.format(p['wage'])}원 · ${p['label']}',
      ...
      if (_customWage) ...[
        const SizedBox(height: 12),
        TextField(
          controller: _wageCtrl, keyboardType: TextInputType.number,
          autofocus: true,
          decoration: InputDecoration(
            hintText: '예) 13000', suffixText: '원/시간',
        ...
        if (_generateError != null)
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 8),
              Text(_generateError!, style: const TextStyle(fontSize: 12, color: Colors.red)),
            ]),
          ),
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 10),
        const Text('직접 입력',
            style: TextStyle(fontSize: 14, color: _sub, fontWeight: FontWeight.w600)),
      ]),
    ),
  );
}