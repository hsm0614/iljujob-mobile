// 📁 presentation/screens/post_job/job_preview_detail_screen.dart
...
final results = await Future.wait([
  ...
  String _periodText() {
    if ((widget.startDate != null && widget.startDate!.trim().isNotEmpty) &&
      ...
      DateTime? s, e;
      try { s = DateTime.parse(widget.startDate!.trim()); } catch (_) {}
      try { e = DateTime.parse(widget.endDate!.trim()); }   catch (_) {}
      if (s != null && e != null) {
    ...
    final flatDays = widget.weekdays
        .map((s) => s.trim())
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('공고 미리보기',
          style: TextStyle(fontFamily: 'Jalnan2TTF', fontSize: 20,
        ...
        children: [
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) => ClipRRect(
                  ...
                  child: Image.file(widget.images[i],
                      width: 200, height: 176, fit: BoxFit.cover)),
              ),
            ),
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Expanded(child: Text(widget.title,
                    style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w700,
                        fontFamily: 'Jalnan2TTF', color: _kText))),
                ]),
                const SizedBox(height: 12),
                Row(children: [
                  const Icon(Icons.place_outlined, size: 14, color: _kLabel),
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('근무 정보', style: TextStyle(
                ...
                if (widget.description.trim().isNotEmpty) ...[
                  ...
                  const SizedBox(height: 14),
                  const Text('상세 설명', style: TextStyle(
                      fontWeight: FontWeight.w600, fontSize: 14, color: _kText)),
                  const SizedBox(height: 8),
                  Text(widget.description.trim(),
                    style: const TextStyle(fontSize: 14, height: 1.6, color: _kText)),
                ] else ...[
                  const SizedBox(height: 16),
                  const Divider(height: 1, color: _kBorder),
                  const SizedBox(height: 14),
          ...
          if (widget.lat != 0 && widget.lng != 0)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('근무 위치', style: TextStyle(
                    fontWeight: FontWeight.w700, fontSize: 16, color: _kText)),
                const SizedBox(height: 10),
                ClipRRect(
                  borderRadius: BorderRadius.circular(14),
                  child: SizedBox(height: 200, child: Stack(children: [
                    FlutterMap(
                      options: MapOptions(
                    ...
                    Positioned(left: 8, right: 8, bottom: 8,
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(10)),
                        child: Row(children: [
                          const Icon(Icons.place, size: 14, color: _kLabel),
            ...
            child: Container(
              ...
              child: Row(children: [
                Container(
                  width: 48, height: 48,
                  decoration: BoxDecoration(
                    color: _kBrand.withOpacity(0.08), shape: BoxShape.circle),
                  child: const Icon(Icons.business_outlined,
                      color: _kBrand, size: 22)),
                const SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text(widget.companyName.isNotEmpty
                      ? widget.companyName : '회사명 미입력',
                    style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700,
                        color: _kText)),
                  const SizedBox(height: 4),
                  Text(widget.managerName.isNotEmpty
                      ? '담당자: ${widget.managerName}' : '담당자 정보 없음',
                    style: const TextStyle(fontSize: 13, color: _kLabel)),
                ])),
              ]),
            ),
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                ...[
                  ...
                  child: Row(children: [
                    const Icon(Icons.check_circle_outline, size: 14, color: _kBrand),
                    const SizedBox(width: 6),
                    Text(t, style: const TextStyle(fontSize: 12, color: _kBrand)),
                  ]),
                )),
              ]),
            ),
  ...
  Widget _infoRow(IconData icon, String text) => Row(children: [
    ...
    Expanded(child: Text(text, style: const TextStyle(
        fontSize: 14, color: _kText, fontWeight: FontWeight.w500))),
  ]);
}
...
class _AiInsightCardState extends State<_AiInsightCard> {
  ...
  Widget build(BuildContext context) {
    ...
    if (pi != null && pi.avgHourly > 0) {
      ...
      if (ratio >= 1.2)      { payPosition = '평균보다 높아요 🔥';  payPosColor = Colors.green.shade600; }
      else if (ratio >= 0.95){ payPosition = '평균 수준이에요';      payPosColor = _kBrand; }
      else                   { payPosition = '평균보다 낮아요 ⚠️';  payPosColor = Colors.orange.shade700; }
    }
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: gradeColor.withOpacity(0.35), width: 2.5)),
              child: Center(child: Text('${q.score}',
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                children: [
              Text(q.gradeLabel, style: TextStyle(
                  fontSize: 15, fontWeight: FontWeight.w700, color: gradeColor)),
              const SizedBox(height: 2),
              ClipRRect(
                borderRadius: BorderRadius.circular(99),
                child: LinearProgressIndicator(
                  value: q.score / 100, minHeight: 6,
              ...
              child: Row(children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(6)),
                  child: Text('개선 포인트 ${q.tips.length}개',
...
if (_tipsExpanded)
  Padding(
    ...
    child: Column(children: q.tips.map((tip) => Container(
        ...
        border: Border.all(
          color: tip.impact == 'critical'
              ...
              : tip.impact == 'high'
                  ...
                  : _kBorder)),
      child: Row(children: [
        Text(tip.impact == 'critical' ? '🚨'
            : tip.impact == 'high' ? '⚠️' : '💡',
          style: const TextStyle(fontSize: 14)),
        const SizedBox(width: 8),
        Expanded(child: Text(tip.msg,
          style: const TextStyle(fontSize: 12, color: _kText))),
      ]),
    )).toList()),  // ✅ Column 닫힘
  ),               // ✅ Padding 닫힘
