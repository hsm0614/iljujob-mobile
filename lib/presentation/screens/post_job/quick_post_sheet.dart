...
class QuickPostSheet extends StatelessWidget {
  ...
  static Future<void> show(BuildContext context, {required Map<String, dynamic> job}) {
...
class _QuickPostSheetBodyState extends State<_QuickPostSheetBody> {
  ...
  Future<void> _loadCounts() async {
    ...
    try {
      ...
      if (passRes.statusCode == 200) {
        ...
        final remain = int.tryParse('${d['remaining'] ?? d['remain'] ?? 0}') ?? 0;
        ...
        return ConstrainedBox(
          ...
          child: Column(children: [
            Expanded(child: SingleChildScrollView(
              ...
              child: Column(children: [
                Text(isStart ? '시작일 선택' : '종료일 선택', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ...
                TableCalendar(
                  ...
                  onDaySelected: (d, f) => set(() { selected = DateTime(d.year, d.month, d.day); focused = d; }),
            ...
            SafeArea(
              ...
              child: SizedBox(width: double.infinity,
                child: ElevatedButton(
                  ...
                  child: const Text('선택 완료', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ),
              ),
            ),
          ]),
  ...
  // ─── 급여 검증 ───
  void _validatePay() {
    if (pay <= 0) { setState(() => _payWarning = null); return; }
    // 시간 정보 없으면 스킵
    if (startTime.isEmpty || endTime.isEmpty) { setState(() => _payWarning = null); return; }
    final sParts = startTime.split(':');
    final eParts = endTime.split(':');
    if (sParts.length < 2 || eParts.length < 2) { setState(() => _payWarning = null); return; }
    ...
    setState(() => _payWarning = pay >= required ? null : '💰 최저시급 미달 · 최소 ${_fmt.format(required)}원 이상');
  }

  // ─── 등록 실행 ───
  Future<void> _submit({required bool isPaid}) async {
    if (startDate == null || endDate == null) {
      _showSnack('날짜를 선택해주세요'); return;
    }
    if (pay <= 0) { _showSnack('급여를 입력해주세요'); return; }
    if (_payWarning != null) { _showSnack(_payWarning!); return; }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('공고 등록 완료 🎉'), backgroundColor: Colors.green),
      );

      // 화면 이동
      if (userType == 'client') Navigator.pushNamedAndRemoveUntil(context, '/client_main', (_) => false);
      else Navigator.pushNamedAndRemoveUntil(context, '/home', (_) => false);
      ...
      builder: (ctx) {
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('등록 방식 선택', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: _blue)),
      ...
      child: DraggableScrollableSheet(
        ...
        builder: (_, sc) => Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              Container(
                ...
                child: const Text('⚡ 빠른 등록', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _blue)),
              ),
              const SizedBox(width: 10),
              const Expanded(
                child: Text('날짜·급여만 바꾸고 바로 등록', style: TextStyle(fontSize: 13, color: _label, fontWeight: FontWeight.w500)),
              ),
            ]),
          ),
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // ── 날짜 수정 ──
                _SectionTitle(icon: Icons.calendar_today_rounded, label: '근무 날짜'),
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _DateTile(
                    label: '시작일',
                    value: startDate != null ? _df.format(startDate!) : null,
                    onTap: () => _pickDate(isStart: true),
                  )),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 12),
                    child: Text('→', style: TextStyle(fontSize: 18, color: _label.withOpacity(0.6))),
                  ),
                  Expanded(child: _DateTile(
                    label: '종료일',
                    value: endDate != null ? _df.format(endDate!) : null,
                ...
                if (startDate != null && endDate != null) ...[
                  const SizedBox(height: 8),
                  _DateRangeSummary(start: startDate!, end: endDate!, dfShort: _dfShort),
                ],

                const SizedBox(height: 24),

                // ── 급여 수정 ──
                _SectionTitle(icon: Icons.payments_rounded, label: '급여'),
                const SizedBox(height: 10),
                ...
                Row(children: ['일급', '주급'].map((t) {
                  ...
                  return Expanded(child: Padding(
                    padding: EdgeInsets.only(right: t == '일급' ? 8 : 0),
                    child: GestureDetector(
                      onTap: () { setState(() => payType = t); _validatePay(); },
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    hintText: '0',
                    hintStyle: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _border),
                    suffixText: '원',
                    suffixStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _label),
                    filled: true, fillColor: _bg,
                    contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
                    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: _payWarning != null ? Colors.red : _border)),
                    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: _payWarning != null ? Colors.red : _blue, width: 1.5)),
                  ...
                  onChanged: (v) {
                    ...
                    setState(() => pay = parsed);
                    _validatePay();
                    final formatted = num.isEmpty ? '' : _fmt.format(parsed);
                    if (_payCtrl.text != formatted) {
                      _payCtrl.value = TextEditingValue(text: formatted, selection: TextSelection.collapsed(offset: formatted.length));
                    }
                  },
                ),
                ...
                if (_payWarning != null) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: const Color(0xFFFFF3F0), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.red.shade200)),
                    child: Row(children: [
                      const Text('⚠️', style: TextStyle(fontSize: 13)),
                      const SizedBox(width: 6),
                      Expanded(child: Text(_payWarning!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.red))),
                    ]),
                  ),
                ],
                ...
                const SizedBox(height: 8),
                Row(children: [
                  const Icon(Icons.info_outline, size: 13, color: _label),
                  const SizedBox(width: 4),
                  Text('2026년 최저시급 ${_fmt.format(_minWage)}원', style: const TextStyle(fontSize: 11, color: _label)),
                ]),
          ...
          _BottomButtons(
            ...
            canSubmit: startDate != null && endDate != null && pay > 0 && _payWarning == null,
...
class _JobSummaryCard extends StatelessWidget {
  ...
  const _JobSummaryCard({required this.title, required this.category, required this.location, required this.startTime, required this.endTime, required this.isSameDayPay});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // 제목 + 카테고리
        Row(children: [
          Expanded(child: Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _text), maxLines: 1, overflow: TextOverflow.ellipsis)),
          const SizedBox(width: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(color: _blue.withOpacity(0.12), borderRadius: BorderRadius.circular(6)),
            child: Text(category, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _blue)),
          ),
        ]),
        const SizedBox(height: 10),
        ...
        Wrap(
          ...
          children: [
            ...
            if (startTime.isNotEmpty && endTime.isNotEmpty)
              _InfoChip(icon: Icons.access_time_rounded, label: '$startTime ~ $endTime'),
            if (isSameDayPay)
              _InfoChip(icon: Icons.attach_money_rounded, label: '당일지급', color: Colors.green),
          ],
        ),
        ...
        const SizedBox(height: 10),
        Row(children: [
          Icon(Icons.check_circle_outline, size: 14, color: _blue.withOpacity(0.7)),
          const SizedBox(width: 4),
          const Text('위 정보를 그대로 재사용해요', style: TextStyle(fontSize: 11, color: _label, fontWeight: FontWeight.w500)),
        ]),
      ]),
    );
  }
...
class _InfoChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [
    ...
    Text(label, style: TextStyle(fontSize: 12, color: color ?? _sub, fontWeight: FontWeight.w500)),
  ]);
}
...
class _DateRangeSummary extends StatelessWidget {
  ...
  const _DateRangeSummary({required this.start, required this.end, required this.dfShort});
...
class _SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 6),
    Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _text)),
  ]);
}
...
class _DateTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: value != null ? _blue : _label)),
        const SizedBox(height: 4),
        Text(value ?? '선택', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: value != null ? _text : _label)),
      ]),
    ),
  );
}
...
class _OptionCard extends StatelessWidget {
  ...
  final bool badgeOk;
  final VoidCallback onTap;
  const _OptionCard({required this.emoji, required this.title, required this.desc, required this.badge, required this.badgeOk, required this.onTap});
    ...
    child: Container(
      ...
      child: Row(children: [
        ...
        Container(
          ...
          child: Text(badge, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: badgeOk ? _blue : Colors.red)),
        ),
      ]),
    ),
  );
...
class _BottomButtons extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _BottomButtons({required this.isSubmitting, required this.canSubmit, required this.onTap});
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: isSubmitting
                ...
                : const Text('등록 방식 선택 →', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, letterSpacing: -0.2)),
          ),
        ),
      ),
    );
