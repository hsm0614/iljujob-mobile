// 📁 lib/presentation/screens/client_screen/wage_report_screen.dart
...
class _WageReportScreenState extends State<WageReportScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('임금 AI 리포트', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 17)),
        actions: [
          IconButton(
  ...
  Widget _buildBody() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          // ── 헤더 ─────────────────────────────────────────────────
          _sectionHeader('📊 ${r.category} 급여 시장 분석', '${r.locationCity} · ${r.sampleLocal}건 분석'),
          const SizedBox(height: 12),
          ...
          _card(children: [
            ...
            Row(children: [
              ...
              _statItem('추천시급', r.recommendedHourly, const Color(0xFF16A34A)),
            ]),
          ]),
          const SizedBox(height: 12),
          ...
          _card(children: [
            Row(children: [
              Container(
                ...
                child: const Text('AI 분석', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)),
              ),
              const SizedBox(width: 8),
              const Text('Gemini AI 리포트', style: TextStyle(fontSize: 12, color: Colors.grey)),
            ]),
          ...
          Center(
            child: Text(
              '* 알바일주 DB 기반 분석 · ${r.cached ? '캐시' : '실시간'} · 최저임금 10,030원 기준',
  ...
  Widget _payRangeBar(WageReport r) {
    ...
    final currentRatio = r.currentHourly != null
        ? ((r.currentHourly! - r.minHourly) / total).clamp(0.0, 1.0)
        : null;
    final avgRatio   = ((r.avgHourly   - r.minHourly) / total).clamp(0.0, 1.0);
    final recRatio   = ((r.recommendedHourly - r.minHourly) / total).clamp(0.0, 1.0);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('시장 급여 분포', style: TextStyle(fontSize: 12, color: Colors.grey, fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
          ...
          return Stack(
            ...
            children: [
              ...
              Positioned(
                ...
                top: -4,
                child: Container(width: 2, height: 16, color: const Color(0xFF1D4ED8)),
              ),
              // 추천 마커
              Positioned(
        ...
        Row(
          ...
          children: [
            Text('${_fmt.format(r.minHourly)}원', style: const TextStyle(fontSize: 10, color: Colors.grey)),
            Text('${_fmt.format(r.maxHourly)}원', style: const TextStyle(fontSize: 10, color: Colors.grey)),
          ],
        ),
      ],
    );
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('현재 시급 ${_fmt.format(r.currentHourly)}원',
                  style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
              const SizedBox(height: 2),
              Text('시장 내 $label (${pct != null ? '$pct 퍼센타일' : ''})',
                  style: TextStyle(fontSize: 13, color: color)),
              if (r.applyRateUplift != null && r.applyRateUplift! > 0) ...[
                const SizedBox(height: 2),
                Text('추천 시급으로 올리면 지원자 +${r.applyRateUplift}% 예상',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('AI 추천 급여', style: TextStyle(color: Colors.white70, fontSize: 12)),
              Text(
          ...
          Container(
            ...
            child: const Text('적용하기', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
  ...
  Widget _statItem(String label, int value, Color color) {
    return Expanded(
      child: Column(children: [
        Text('${_fmt.format(value)}원', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: color)),
        const SizedBox(height: 2),
