...
import 'package:iljujob/presentation/screens/job_insight_sheet.dart';
...
class _ClientHomeScreenState extends State<ClientHomeScreen>
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.delete_rounded, color: Color(0xFFE53935), size: 24),
              ),
              const SizedBox(height: 12),
              const Text('공고 삭제', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
              const SizedBox(height: 8),
              const Text(
                '해당 공고를 삭제하시겠습니까?\n삭제 후에는 복구할 수 없습니다.',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 13.5, height: 1.35, color: Color(0xFF666666)),
              ),
  ...
  Future<void> _saveClientFcmToken() async {
    ...
    try {
      ...
      await http.post(
        ...
        body: jsonEncode({'userPhone': phone, 'userType': 'client', 'fcmToken': token}),
      );
    } catch (e) {
  ...
  Future<void> _fetchClientProfile() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (!mounted) return;
        setState(() { isSafeCompany = certUrl != null && certUrl.isNotEmpty; });
      }
    } catch (e) {
  ...
  Future<void> _loadMyJobs({int? page}) async {
    ...
    try {
      ...
      if (clientId == null) { _toast('로그인 정보가 확인되지 않습니다.'); return; }
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final jobs = (data['jobs'] as List).map((json) => Job.fromJson(json)).toList();
        if (!mounted) return;
        setState(() {
          myJobs      = jobs;
                ...
                return GestureDetector(
                  ...
                  child: DecoratedBox(
                    ...
                    child: Image.network(
                      ...
                      errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.image_not_supported_outlined, color: Colors.black38)),
                    ),
                  ),
                );
              },
            ...
            child: ClipOval(
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: const SizedBox(width: 28, height: 28, child: Icon(Icons.close, size: 16, color: Colors.white)),
                ),
              ),
            ),
          ),
  ...
  Future<void> _openRecommendedWorkersByJobId(String jobIdStr) async {
    ...
    if (jid == null) { _toast('공고 ID가 올바르지 않습니다.'); return; }
    ...
    try {
      ...
      if (!isSubscribed) { await _showPaywall(); return; }
      if (!mounted) return;
      ...
      await showModalBottomSheet(
        ...
        builder: (_) => FractionallySizedBox(
          ...
          child: ClipRRect(
            ...
            child: Material(color: Colors.white, child: RecommendedWorkersSheet(api: AiApi(baseUrl), jobId: jid)),
          ),
        ),
      );
    } catch (e) {
            ...
            child: SafeArea(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  Container(
                    width: 44, height: 44,
                    decoration: BoxDecoration(color: const Color(0xFF4F46E5).withOpacity(0.10), borderRadius: BorderRadius.circular(14)),
                    child: const Icon(Icons.auto_awesome, color: Color(0xFF4F46E5)),
                  ),
                  const SizedBox(height: 10),
                  const Text('맞춤 인재 기능은 구독 전용입니다', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 8),
                  const Text('AI가 공고와 잘 맞는 인재를 추천해 드립니다.\n구독 후 이용해 주세요.', textAlign: TextAlign.center, style: TextStyle(color: Colors.black54)),
                  ...
                  Row(children: [
                    ...
                    Expanded(child: ElevatedButton(
                      ...
                      onPressed: () { Navigator.pop(ctx); Navigator.pushNamed(context, '/subscribe'); },
      ...
      return Expanded(
        child: Container(
          ...
          child: Row(children: [
            Container(
              width: 34, height: 34,
              decoration: BoxDecoration(color: kBrandBlue.withOpacity(0.10), borderRadius: BorderRadius.circular(10)),
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: const Icon(Icons.verified_user_outlined, color: Colors.orange, size: 18),
          ),
          const SizedBox(width: 10),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('안심기업 인증', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w900)),
            SizedBox(height: 2),
            Text('인증을 완료하시면 지원 전환율이 올라갈 수 있습니다.', style: TextStyle(fontSize: 12, color: Colors.black54)),
          ])),
          const SizedBox(width: 10),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
            ...
            child: const Text('인증하기', style: TextStyle(fontWeight: FontWeight.w800)),
          ),
        ]),
      ),
    );
  ...
  Widget _buildStatusSegment() {
    Widget seg(String label) {
      ...
      return Expanded(
        child: InkWell(
          ...
          onTap: () { setState(() => filterStatus = label); _onFilterChanged(); },
          child: Container(
            padding: const EdgeInsets.symmetric(vertical: 10),
            decoration: BoxDecoration(
              color: selected ? kBrandBlue.withOpacity(0.12) : Colors.transparent,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: selected ? kBrandBlue.withOpacity(0.35) : Colors.transparent),
            ),
            child: Center(child: Text(label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w900, color: selected ? kBrandBlue : Colors.black54))),
          ),
        ),
      );
    }
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.grey.shade200)),
        child: Row(children: [seg('전체'), const SizedBox(width: 6), seg('공고중'), const SizedBox(width: 6), seg('마감')]),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(children: [
        PopupMenuButton<String>(
          tooltip: '정렬',
          onSelected: (v) { setState(() => sortType = v); _onFilterChanged(); },
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Text(sortType, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800)),
        ...
        Container(
          ...
          child: IconButton(
            ...
            icon: Icon(compactView ? Icons.view_agenda_outlined : Icons.view_list_outlined),
  ...
  Widget _buildPaginationWidget() {
    ...
    return Container(
      ...
      child: Column(children: [
        Text('총 ${totalCount}개 공고 · ${currentPage}/${totalPages}페이지',
          style: const TextStyle(fontSize: 12, color: Colors.black54, fontWeight: FontWeight.w600)),
        const SizedBox(height: 10),
        SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          child: Row(children: [
            _pageIconBtn(Icons.first_page,    enabled: currentPage > 1,           onTap: () => _goToPage(1)),
            _pageIconBtn(Icons.chevron_left,  enabled: currentPage > 1,           onTap: () => _goToPage(currentPage - 1)),
            ..._buildPageNumbers(),
            _pageIconBtn(Icons.chevron_right, enabled: currentPage < totalPages,  onTap: () => _goToPage(currentPage + 1)),
            _pageIconBtn(Icons.last_page,     enabled: currentPage < totalPages,  onTap: () => _goToPage(totalPages)),
          ]),
        ),
      ]),
    );
  ...
  Widget _pageIconBtn(IconData icon, {required bool enabled, required VoidCallback onTap}) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 2),
      child: SizedBox(width: 38, height: 38, child: IconButton(onPressed: enabled ? onTap : null, icon: Icon(icon, size: 18), splashRadius: 20)),
    );
  }
  ...
  List<Widget> _buildPageNumbers() {
    ...
    if (startPage > 1) {
      ...
      if (startPage > 2) pageButtons.add(const Padding(padding: EdgeInsets.symmetric(horizontal: 6), child: Text('...', style: TextStyle(color: Colors.black38))));
    }
    for (int i = startPage; i <= endPage; i++) pageButtons.add(_pageNumBtn(i));
    if (endPage < totalPages) {
      if (endPage < totalPages - 1) pageButtons.add(const Padding(padding: EdgeInsets.symmetric(horizontal: 6), child: Text('...', style: TextStyle(color: Colors.black38))));
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: isCurrent ? kBrandBlue : Colors.grey.shade200),
          ),
          child: Text('$page', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: isCurrent ? Colors.white : Colors.black54)),
        ),
      ),
    );
  }
  ...
  Widget _badge(String text, {required Color color, IconData? icon}) {
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(text, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: color, height: 1.0)),
      ]),
    );
  }
  ...
  Widget _metaLine(IconData icon, String text) {
    return Row(children: [
      ...
      const SizedBox(width: 8),
      Expanded(child: Text(text, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, color: Colors.black87, fontWeight: FontWeight.w600))),
    ]);
  }
  ...
  Future<void> _openJobActions(Job job) async {
    ...
    final v = await showModalBottomSheet<String>(
      ...
      builder: (_) => SafeArea(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(width: 36, height: 4, decoration: BoxDecoration(color: Colors.black12, borderRadius: BorderRadius.circular(99))),
            const SizedBox(height: 10),
            ListTile(leading: const Icon(Icons.people_outline), title: const Text('지원자 보기', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'applicants')),
            ListTile(leading: const Icon(Icons.info_outline), title: const Text('상세 보기', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'detail')),
            if (!isClosed)
              ListTile(leading: const Icon(Icons.edit_outlined), title: const Text('수정', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'edit')),
            if (!isClosed)
              ListTile(leading: const Icon(Icons.auto_awesome, color: Color(0xFF4F46E5)), title: const Text('맞춤 인재', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'recommend')),
            ListTile(leading: const Icon(Icons.insights, color: kBrandBlue), title: const Text('인사이트', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'insight')),
            if (reserved)
              ListTile(leading: Icon(Icons.flash_on, color: Colors.orange.shade700), title: const Text('즉시 게시', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'publish-now')),
            if (isClosed)
              ListTile(leading: const Icon(Icons.replay_circle_filled), title: const Text('재공고', style: TextStyle(fontWeight: FontWeight.w800)), onTap: () => Navigator.pop(context, 'repost')),
            const Divider(height: 18),
            ListTile(leading: const Icon(Icons.delete_outline, color: Colors.red), title: const Text('삭제', style: TextStyle(fontWeight: FontWeight.w900, color: Colors.red)), onTap: () => Navigator.pop(context, 'delete')),
          ]),
        ),
      ),
    );
    ...
    switch (v) {
      case 'edit':        Navigator.pushNamed(context, '/edit_job', arguments: job.id); break;
      case 'detail':      Navigator.pushNamed(context, '/job-detail', arguments: job); break;
      case 'applicants':  Navigator.pushNamed(context, '/applicants', arguments: job.id); break;
      case 'recommend':   _openRecommendedWorkersByJobId(job.id.toString()); break;
      case 'insight':
        final jobId = int.tryParse(job.id.toString());
        if (jobId != null) JobInsightSheet.show(context, jobId: jobId, jobTitle: job.title);
        break;
      case 'publish-now':
        try {
          ...
          _loadMyJobs();
        } catch (_) { _toast('즉시 게시에 실패했습니다.'); }
        break;
      case 'repost':
        Navigator.pushNamed(context, '/post_job', arguments: {'isRepost': true, 'existingJob': job});
        break;
      case 'delete':
        // ① 단일 메서드 호출
  ...
  Widget _buildCompactJobCard(Job job) {
    ...
    final payTypeColor = job.payType == '주급' ? Colors.green.shade700 : Colors.deepOrange.shade700;
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(job.title, maxLines: 1, overflow: TextOverflow.ellipsis,
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900, color: isClosed ? Colors.black38 : Colors.black, decoration: isClosed ? TextDecoration.lineThrough : null))),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                decoration: BoxDecoration(color: payTypeColor.withOpacity(0.10), borderRadius: BorderRadius.circular(999), border: Border.all(color: payTypeColor.withOpacity(0.30))),
                child: Text(job.payType, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: payTypeColor)),
              ),
              const SizedBox(width: 4),
              IconButton(tooltip: '관리', onPressed: () => _openJobActions(job), icon: const Icon(Icons.more_horiz, color: Colors.black54)),
            ]),
            const SizedBox(height: 8),
            Wrap(spacing: 6, runSpacing: 6, children: [
              if (reserved) _badge('예약 게시', color: Colors.orange.shade700, icon: Icons.schedule_outlined),
              if (pinned)   _badge('상단 고정', color: Colors.deepOrange.shade700, icon: Icons.push_pin_outlined),
              if (pinned)   _badge(pinnedRemainText(job), color: Colors.deepOrange.shade700),
              if (isClosed) _badge('마감', color: Colors.grey.shade700, icon: Icons.stop_circle_outlined),
              if (job.expiresAt != null && !isClosed) _badge(getExpiryText(job), color: Colors.red.shade700, icon: Icons.access_time),
            ]),
            const SizedBox(height: 10),
      ...
      child: Container(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(job.title, style: titleStyle, maxLines: 1, overflow: TextOverflow.ellipsis)),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                  decoration: BoxDecoration(color: payTypeColor.withOpacity(0.10), borderRadius: BorderRadius.circular(999), border: Border.all(color: payTypeColor.withOpacity(0.30))),
                  child: Text(job.payType, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: payTypeColor)),
              ...
              if (reserved || pinned || isClosed || (job.expiresAt != null && !isClosed)) ...[
                Wrap(spacing: 6, runSpacing: 6, children: [
                  if (reserved) _badge('예약 게시', color: Colors.orange.shade700, icon: Icons.schedule_outlined),
                  if (pinned)   _badge('상단 고정', color: Colors.deepOrange.shade700, icon: Icons.push_pin_outlined),
                  if (pinned)   _badge(pinnedRemainText(job), color: Colors.deepOrange.shade700),
              ...
              if (job.description?.isNotEmpty == true) ...[
                const SizedBox(height: 10),
                Text(job.description!, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black54, fontSize: 13, height: 1.35)),
              ],
              const SizedBox(height: 12),
              ...
              Wrap(
                ...
                children: [
                  _actionBtn(
                    icon: Icons.people_outline, label: '지원자',
                    onTap: () => Navigator.pushNamed(context, '/applicants', arguments: job.id),
                  ),
                  _actionBtn(
                    icon: Icons.insights, label: '인사이트', color: kBrandBlue,
                    onTap: () {
                      final jobId = int.tryParse(job.id.toString());
                      if (jobId != null) JobInsightSheet.show(context, jobId: jobId, jobTitle: job.title);
                    },
                  ...
                  if (isClosed)
                    _actionBtn(
                      icon: Icons.replay_circle_filled, label: '재공고', color: kBrandBlue,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('사장님 공고 관리',
          style: TextStyle(fontFamily: 'Jalnan2TTF', color: kBrandBlue, fontSize: 20, fontWeight: FontWeight.w900)),
        bottom: TabBar(
        ...
        child: CustomScrollView(
          slivers: [
            ...
            if (isLoading)
              ...
              const SliverFillRemaining(hasScrollBody: false, child: Center(child: Text('등록하신 공고가 없습니다.')))
            else ...[
              SliverList.builder(
        ...
        child: InkWell(
          ...
          child: Ink(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: const [
              ...
              SizedBox(width: 8),
              Text('공고 등록', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w900, height: 1.0, letterSpacing: -0.2)),
            ]),
          ),
        ),
      ),
