...
Future<void> _init() async {
  ...
  final fLoc = _prepareLocation()
      .then((_) => debugPrint('[_init] ✅ location done lat=$currentLatitude lon=$currentLongitude'))
      .catchError((e) => debugPrint('[_init] ❌ location err: $e'));

  final fJobs = _loadJobs()
      .then((_) => debugPrint('[_init] ✅ loadJobs done all=${allJobs.length} filtered=${filteredJobs.length}'))
      .catchError((e) => debugPrint('[_init] ❌ loadJobs err: $e'));

  try {
    debugPrint('[_init] ⏳ waiting location & jobs in parallel…');
    ...
    unawaited(
      _fetchRemainingPass()
          ...
          .then((_) => debugPrint('[_init] 🔸 pass fetched remaining=$_remainingPass'))
          .catchError((e) => debugPrint('[_init] ❌ pass fetch err: $e'))
    );
  } finally {
    final t1 = DateTime.now();
    debugPrint('🔴 [_init] end inst=$inst dt=${t1.difference(t0).inMilliseconds}ms mounted=$mounted');
    if (!mounted) return;
...
Future<void> _loadJobs() async {
  ...
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('네트워크가 지연됩니다. 다시 시도해주세요.')),
    );
  } catch (e, st) {
    if (!mounted || seq != _loadSeq) return;
    if (allJobs.isEmpty) {
...
Future<void> _fetchRemainingPass() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('❌ 이용권 조회 실패: ${response.body}');
    }
  } catch (e) {
    debugPrint('❌ 네트워크 오류(_fetchRemainingPass): $e');
  }
}
